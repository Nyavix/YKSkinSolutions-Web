...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    // Pause autoplay — we drive time via scroll
    video.pause();
    ...
    const onScroll = () => {
      ...
      // Progress: 0 when section top is at viewport top, 1 when section bottom leaves viewport top
      const scrolled = -top; // how many px the user has scrolled into the section
    ...
    const onLoaded = () => {
      // Run once on load to set frame 0
      onScroll();
    };
  ...
  return (
    <section
      ref={sectionRef}
      id="hero"
      className="relative flex flex-col justify-center items-center text-center px-6 pt-32 pb-20 overflow-hidden"
      style={{ minHeight: '200vh' }} // tall section so there's scroll room
    >
      {/* ── Video background ── */}
      <video
        ref={videoRef}
      ...
      <div
        ...
        style={{
          zIndex: 1,
          background:
            'linear-gradient(160deg, rgba(253,242,248,0.82) 0%, rgba(255,247,251,0.75) 50%, rgba(253,232,244,0.82) 100%)',
        }}
      />
      ...
      <div aria-hidden="true" className="absolute top-[-10%] right-[-5%] w-80 h-80 rounded-full pointer-events-none"
        style={{ zIndex: 2, background: 'radial-gradient(circle, rgba(249,168,212,0.4) 0%, transparent 70%)', filter: 'blur(40px)' }} />
      <div aria-hidden="true" className="absolute bottom-[5%] left-[-5%] w-96 h-96 rounded-full pointer-events-none"
        style={{ zIndex: 2, background: 'radial-gradient(circle, rgba(139,92,246,0.1) 0%, transparent 70%)', filter: 'blur(50px)' }} />

      {/* ── Sticky content panel ── scrolls with user then sticks at top */}
      <div
        className="sticky top-0 flex flex-col justify-center items-center text-center"
      ...
      >
        <div className="relative max-w-3xl mx-auto px-6">
          ...
          {/* Trust badges */}
          <div className="flex flex-wrap justify-center gap-6 mt-14 pt-10" style={{ borderTop: '1px solid rgba(251,207,232,0.6)' }}>
          ...
          {/* Scroll cue */}
          <div className="mt-12 flex flex-col items-center gap-1" aria-hidden="true">
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#EC4899" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
              style={{ animation: 'hero-bounce 1.8s infinite' }}>
