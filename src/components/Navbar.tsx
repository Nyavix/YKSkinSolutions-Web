  ...
  return (
    <>
      <header className="fixed top-4 left-4 right-4 z-50">
        ...
        >
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 flex-shrink-0" aria-label="Yellowknife Skin Solutions Home">
            ...
            <span className="text-sm font-semibold tracking-tight" style={{ fontFamily: 'var(--font-playfair, serif)', color: '#831843' }}>
              YK Skin Solutions
            </span>
          </Link>
              ...
              return (
                <li key={link.href}>
                  <Link
                    ...
                    style={{ color: isActive ? '#EC4899' : '#9D174D', borderBottom: isActive ? '2px solid #EC4899' : '2px solid transparent' }}
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          <div className="flex items-center gap-2">
            {/* Book Now — desktop only, wrapped so .btn-primary display doesn't fight Tailwind hidden */}
            <div className="hidden md:block">
              <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="btn-primary text-sm py-2.5 px-5">
                Book Now
              </a>
            </div>
            ...
            <button
              id="nav-burger"
              className="md:hidden flex flex-col justify-center items-center gap-1.5 w-10 h-10 rounded-xl flex-shrink-0"
              style={{ background: menuOpen ? 'rgba(236,72,153,0.1)' : 'transparent', cursor: 'pointer' }}
            ...
            >
              <span className="block w-5 h-0.5 rounded-full transition-all duration-300 origin-center"
                style={{ background: '#831843', transform: menuOpen ? 'translateY(5px) rotate(45deg)' : 'none' }} />
              <span className="block w-5 h-0.5 rounded-full transition-all duration-300"
                style={{ background: '#831843', opacity: menuOpen ? 0 : 1 }} />
              <span className="block w-5 h-0.5 rounded-full transition-all duration-300 origin-center"
                style={{ background: '#831843', transform: menuOpen ? 'translateY(-5px) rotate(-45deg)' : 'none' }} />
            </button>
          </div>
      ...
      {/* Mobile drawer */}
      <div id="mobile-menu" aria-hidden={!menuOpen} className="md:hidden fixed inset-0 z-40 transition-all duration-300"
        style={{ pointerEvents: menuOpen ? 'auto' : 'none' }}>
        {/* Backdrop */}
        <div className="absolute inset-0 transition-opacity duration-300"
          style={{ background: 'rgba(131,24,67,0.15)', backdropFilter: 'blur(4px)', opacity: menuOpen ? 1 : 0 }}
          onClick={() => setMenuOpen(false)} aria-hidden="true" />

        {/* Panel */}
        <div className="absolute top-0 left-4 right-4 rounded-b-3xl shadow-2xl overflow-hidden transition-all duration-300"
          style={{
          ...
          <nav aria-label="Mobile navigation">
            <ul className="flex flex-col list-none m-0 p-0 px-6 pb-2">
              {navLinks.map((link, i) => {
                ...
                return (
                  <li key={link.href}>
                    <Link href={link.href} id={`mobile-nav-${link.label.toLowerCase()}`}
                      className="flex items-center justify-between py-4 text-base font-medium"
                      style={{ color: isActive ? '#EC4899' : '#831843', borderBottom: i < navLinks.length - 1 ? '1px solid #FBCFE8' : 'none', cursor: 'pointer' }}>
                      {link.label}
                      {isActive
                        ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#EC4899" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M20 6L9 17l-5-5" /></svg>
                        : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9D174D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M9 18l6-6-6-6" /></svg>}
                    </Link>
                  </li>
                );
              })}
